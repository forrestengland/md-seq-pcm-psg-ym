...
#define VDP_DATA_PORT    ((volatile unsigned short*) 0xC00000)
#define VDP_CTRL_PORT    ((volatile unsigned long*)  0xC00004)

/**
 * Sets a tile's attributes at a specific VRAM address.
 * @param vram_addr  The target address in VRAM (e.g., Plane A nametable).
 * @param tile_index The ID of the 8x8 tile in VRAM (0-2047).
 * @param palette    The palette index (0-3).
 */
void set_tile_palette(uint16_t plan, unsigned short tile_index, unsigned char palette, uint16_t x, uint16_t y) {

  uint32_t addr = plan + ((x + (y << PLAN_WIDTH_SFT)) << 1);
  
    // 1. Prepare the VDP Write Command for VRAM
    // Format: (0x4000 + (addr & 0x3FFF)) << 16 | (addr >> 14)
    unsigned long command = ((0x4000 | (addr & 0x3FFF)) << 16) | (addr >> 14);
    
    // 2. Send the write command to the VDP Control Port
    *VDP_CTRL_PORT = command;

    // 3. Construct the 16-bit Tile Attribute word
    // Palette index is stored in bits 14-13
    unsigned short attribute = (tile_index & 0x07FF) | ((palette & 0x03) << 13);
    
    // 4. Write the attribute word to the VDP Data Port
    *VDP_DATA_PORT = attribute;
}
